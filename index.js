...
const {MQTT_HOST, MQTT_USER, MQTT_PASS, AWS_ACCESS_KEY, AWS_SECRET_ACCESS_KEY, AWS_IOT_ENDPOINT_HOST, AWS_REGION, AWS_SUBSCRIBE, MQTT_SUBSCRIBE} = process.env
...
awsMqttClient.on('message', (topic, message) =>
  mqttClient.publish(topic, message, {qos: 2}, err =>
    console.log("aws", err, topic, message.toString())
  ))
...
mqttClient.on('message', (topic, message) =>
  awsMqttClient.publish(topic, message, {qos: 1}, err =>
    console.log("aws", err, topic, message.toString())
  ))
